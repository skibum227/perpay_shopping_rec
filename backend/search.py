...
class CosineSearch:
    ...
    # Business features exposed to the frontend sliders:
    # - profitability: prefer higher margin (0..1 fraction)
    # - return_rate: prefer LOWER return rate (we invert it in scoring)
    DEFAULT_FEATURE_MAP = {
        "profitability": ("profitability", None),  # <— use public column
        "return_rate": ("return_rate", "inverse"),
    }
    ...
    def __init__(self, csv_path: str, *, name_col: str = "name"):
        ...
        if "_derived_margin" in self.df.columns and "current_margin" in self.df.columns:
            if pd.isna(self.df["_derived_margin"]).all():
                self.df["_derived_margin"] = pd.to_numeric(
                    self.df["current_margin"], errors="coerce"
                )

        # Public profitability column (0–1 float), plus numeric return_rate
        if "_derived_margin" in self.df.columns:
            self.df["profitability"] = pd.to_numeric(
                self.df["_derived_margin"], errors="coerce"
            )
        elif "current_margin" in self.df.columns:
            self.df["profitability"] = pd.to_numeric(
                self.df["current_margin"], errors="coerce"
            )
        else:
            self.df["profitability"] = np.nan

        # if "return_rate" in self.df.columns:
        #     self.df["return_rate"] = pd.to_numeric(self.df["return_rate"], errors="coerce")

        if "return_rate" in self.df.columns:
            self.df["return_rate"] = (
                pd.to_numeric(self.df["return_rate"], errors="coerce").fillna(0.0)
            )    
    ...
    def search(
        ...
        *,
        ...
        # Business score (profitability + return_rate)
        biz = self._compute_biz(idx_all, biz_weights)
    ...
    def _finalize(self, out: pd.DataFrame, include_cols: Optional[List[str]]) -> List[dict]:
        if include_cols is None:
            include_cols = [
                "score", "similarity", "business_score",
                "profitability", "return_rate",
                "product_id", self.name_col, "brand", "current_price", "product_url"
            ]
        include_cols = [c for c in include_cols if c in out.columns]
        result = out[include_cols].copy()

        # round numeric columns for readability
        for col in ("profitability", "return_rate", "similarity", "business_score", "score"):
